...
def _recommendation_section(
    ...
    gate_empty = (
        "None in the supplied demonstration inputs."
        if is_demo
        else "None in the supplied run inputs."
    )
    info_empty = (
        "None identified in the supplied demonstration inputs."
        if is_demo
        else "None identified in the supplied run inputs."
    )
    lines = [
...
def render_markdown(run: RecommendationRun) -> str:
    ...
    condition_time_label = "Synthetic condition time" if is_demo else "Condition time"
    preferences_header = "Demonstration preferences" if is_demo else "User preferences"
    ...
    lines.extend(
        [
            ...
            f"- **{condition_time_label}:** {condition.valid_at.isoformat()}",
